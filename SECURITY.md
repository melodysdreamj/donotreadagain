...
| 6 | **Leakage on share** — sharing a file leaks the embedded transcript / summary / entities | `dnr strip` removes the record before sharing; content is unchanged. Sensitive files SHOULD be reviewed/stripped before leaving the trust boundary. | `strip` implemented + tested |
| 7 | **Chain-of-custody** — embedding mutates an original evidence file | Embedding is never done in place (atomic temp+rename); signed/read-only files are out of scope; evidentiary corpora SHOULD use `--no-embed` db-only records so originals remain byte-identical. | — |
