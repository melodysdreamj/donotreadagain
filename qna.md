...
dnr is a **source-file transcript cache protocol plus cache/trust/index layer**. The transcript is an *input*, produced by whoever is best placed: the calling agent's own vision model (`dnr record`), local extractors (PyMuPDF/pypdf, python-docx, openpyxl), local ASR (faster-whisper), or an API. dnr owns the *contract* (verbatim guide) and the *trust/provenance* layer. This keeps it neutral and lets fidelity improve independently (Q8). It is not a general knowledge-base format; it stores faithful text tied back to concrete source files.
