...
**No.** No per-folder note. Each file self-describes (`_about`); an agent fetches the skill (`SKILL.md`) **once** from the canonical URL and then knows dnr in *every* folder. `dnr init` now only ensures a signing key. Nothing is written into the user's folders.
